...
client.on('ready', () => {
  ...
  client.users.cache.get(process.env.ADMIN) ? console.log("LAAAA J'Y SUIS PUTAAAAAIN") : null;
  ...
  // client.users.cache.get(process.env.ADMIN) // get user with id = process.env.ADMIN
  //   .createDM() // create direct message channel with the user
  //   .then(e => e.send('Bonjour je suis UP et prèt a développer...')); // then send a message to the user
});
